...
export default {
  ...
  methods: {
    ...
    async validateSearch() {
      ...
      if (this.cupon == null || this.cupon == '') {
        this.$notify({
          group: 'error',
          title: this.$t('cupon_input'),
          type: 'error'
        })
        return
      }
      const params = {'boleto':this.cupon}
      const response = await APICuponDescuento.validaCuponNuevo(params)
      if (response.data.status){
        console.log('VALIDACION????: '+response.data.response.boleto)
        this.$notify({
          group: 'info',
          title: this.$t('validated_coupon'),
          type: 'info'
        })
        await localStorage.setItem('cuponDescuento',JSON.stringify(response.data))
      } else {this.$notify({
          group: 'error',
          title: this.$t('coupon_validation_fail'),
          type: 'error',
